...
import (
	...
	"github.com/essblock/go-sdk/support/log"
)
...
func build(pkg, dest, version, buildOS, buildArch string) {
	...
	timeFlag := fmt.Sprintf("-X github.com/ess/go/support/app.buildTime=%s", buildTime)
	versionFlag := fmt.Sprintf("-X github.com/ess/go/support/app.version=%s", version)
