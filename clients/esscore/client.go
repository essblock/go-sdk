...
import (
	...
	proto "github.com/essblock/go-sdk/protocols/esscore"
	"github.com/essblock/go-sdk/support/errors"
)

// Client represents a client that is capable of communicating with a
// ess-core server using HTTP
type Client struct {
	// HTTP is the client to use when communicating with ess-core.  If nil,
	// http.DefaultClient will be used.
	HTTP HTTP

	// URL of ess Core server to connect.
	URL string
}

// Info calls the `info` command on the connected ess core and returns the
// provided response
...
// SetCursor calls the `setcursor` command on the connected ess core
func (c *Client) SetCursor(ctx context.Context, id string, cursor int32) error {
	...
	if body != SetCursorDone {
		return errors.Errorf("failed to set cursor on ess-core: %s", body)
	}

	return nil
}

// SubmitTransaction calls the `tx` command on the connected ess core with the provided envelope
...
// WaitForNetworkSync continually polls the connected ess-core until it
...
// simpleGet returns a new GET request to the connected ess-core using the
