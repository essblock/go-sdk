...
// LookupByAddress performs a federated lookup following to the ess
...
// "name" type is a legacy holdover from the legacy ess network's federation
...
// LookupByAccountID performs a federated lookup following to the ess
...
// ForwardRequest performs a federated lookup following to the ess
...
func (c *Client) getFederationServer(domain string) (string, error) {
	etoml, err := c.EssTOML.GetEssToml(domain)
	if err != nil {
		return "", errors.Wrap(err, "get ess.toml failed")
	}

	if etoml.FederationServer == "" {
		return "", errors.New("ess.toml is missing federation server info")
	}

	if !c.AllowHTTP && !strings.HasPrefix(etoml.FederationServer, "https://") {
		return "", errors.New("non-https federation server disallowed")
	}

	return etoml.FederationServer, nil
}
