...
func TestValidateError(t *testing.T) {
	...
	authData := &AuthData{
		Sender:         "bartekess.org",
		NeedInfo:       false,
		Tx:             "&^%",
		AttachmentJSON: "abc",
	}

	assert.EqualError(t, authData.Validate(), "sender: bartekess.org does not validate as ess_address;tx: &^% does not validate as base64;attachment: abc does not validate as json")
}
