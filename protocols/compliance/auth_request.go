...
// to sender server in order to obtain ess.toml file and signing key.
func (r *AuthRequest) VerifySignature(sender string) error {
	...
	senderEssToml, err := esstoml.GetEssTomlByAddress(sender)
	if err != nil {
		return errors.Wrap(err, "Cannot get ess.toml of sender domain")
	}

	if senderEssToml.SigningKey == "" {
		return errors.New("No SIGNING_KEY in ess.toml of sender")
	}

	kp, err := keypair.Parse(senderEssToml.SigningKey)
	if err != nil {
